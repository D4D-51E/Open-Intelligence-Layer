...
**구성 파일**
- `api/copilot.mjs` — 서버 함수(키 은닉). 모드 `summary`(질문형 요약) / `anomaly`(이상징후 서술).
- `src/lib/copilotApi.ts` — 클라이언트 `askCopilot(query, context, history, mode)`.
- `src/components/CopilotPanel.tsx` — 멀티턴 대화 스레드 UI.
- `src/lib/trackFusion.ts` — 클라이언트에서 이미 융합된 지역 상황 JSON 컨텍스트 생성.

**데이터 흐름**
```
사용자 질문 ──▶ CopilotPanel ──▶ askCopilot(query, context, history)
                                     │  context = 현재 뷰포트의 융합 상황
                                     ▼
                             POST /api/copilot  (OPENAI_API_KEY 서버 전용)
                                     │  system: 공개데이터 근거·출처표기·비표적화
                                     ▼
                             OpenAI gpt-4o-mini ──▶ 한국어 브리핑/이상분석 텍스트
```
...
```
Telegram ──POST(update)──▶ /api/tg  (x-telegram-bot-api-secret-token 검증)
                              │  작업·답장 먼저 완료 → 마지막에 200
                              │  (서버리스는 응답 후 함수를 얼려 sendMessage가 잘리므로)
                              ▼
                        명령 라우팅 ──▶ Neon 조회 / OpenAI / 신뢰도 채점 ──▶ sendMessage
```
...
**`/ai` 컨텍스트 빌드(`buildAiContext`)** — 최근 항적 샘플 + 선박 수 + **이름 기반 조회**(질문의 distinctive 토큰을 `name/callsign ILIKE ALL`로 정밀 매칭) + **신뢰도 채점 claims** + 텔레그램 원문. → OpenAI에 전달.
...
```
GitHub Actions cron ──GET /api/tg?push=<job>──▶  (Authorization: Bearer CRON_SECRET)
   bot-briefing (0 */6)  → job=briefing
   bot-rapid   (*/5)     → job=anomaly, job=alert
                              │
                              ▼
                    runPush(job) ─▶ 구독자 조회(bot_subscribers)
                                   ─▶ 항목별 pushItem(dedup_key)  ── 중복이면 skip
                                   ─▶ 신규만 모든 구독자에 sendMessage
```
