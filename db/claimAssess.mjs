// Server-side claim reliability scoring — a faithful JS port of src/lib/claimVerify.ts so the
// Telegram bot (/ai, /verify) scores claims with the SAME logic the web verification panel uses.
// Cross-checks each geolocated Telegram claim against NASA FIRMS thermal hotspots + independent
// cross-source corroboration → verdict + 0–100 confidence + evidence ledger. Keep in sync with
// claimVerify.ts if the scoring changes. Public data, non-targeting.
...
function distKm(aLa, aLo, bLa, bLo) {
  const R = 6371;
...
const decay = (x, scale) => Math.exp(-x / scale);
...
// Geolocate + assess a set of Telegram-like posts ({channelLabel,color,text,date,url}) → sorted claims.
export function assessClaims(posts, thermals, now) {
  const geo = posts
    .map((p) => ({ post: p, loc: geolocate(p.text), t: new Date(p.date).getTime() }))
    .filter((g) => g.loc != null);
  if (!geo.length) return [];
  const corpus = geo.map((g) => ({ channel: g.post.channelLabel, lat: g.loc.lat, lon: g.loc.lon, t: g.t }));
  const seen = new Set();
  const out = [];
  for (const g of geo) {
  ...
  const order = ['TRUE', 'LIKELY', 'NOT LIKELY', 'FALSE'];
